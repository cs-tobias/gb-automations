...
**Bulletproof eligibility** (the CEO's invariant — "only `Utgår` should keep a row off the draft"): every Oppgave under the project lands as a line on the draft unless it falls under one of these five narrow exceptions, ALL of which mean "the row has nothing to add to this run":
...
4. `Type = Korreksjonsrunde` — auto-generated admin sub-rows from the Frame.io integration; never invoiced.
5. Slutt run where the operator dropped `Pris` below the already-billed `Fakturert beløp` — the row is genuinely over-billed; Notion's `Budsjett` vs `Fakturert sum` rollups surface the discrepancy at project level, and a credit-note flow is a future feature, not something this engine should improvise inline.
