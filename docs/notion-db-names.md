...
Pris (number, NOK) — current agreed full price for this deliverable. MUTABLE: the operator can renegotiate between oppstart and slutt, and the engine reads the live value each run (slutt computes `remaining = gross − Fakturert beløp`).
Rabatt (number, Percent format) — per-row discount. Column is modeled as Notion's `Percent` number format so the operator sees the `%` suffix in Notion's UI and types `15` for 15%; Notion's API returns it as the fraction `0.15`, which the engine consumes directly. The engine forwards the rabatt to Fiken on the invoice line itself (`discount` field, as a percent), so the printed invoice shows "Pris kr X / Rabatt Y% / Sum kr Z" rather than just a pre-rabattert unit price. Notion's `Fakturert beløp` and the audit trail record the post-rabatt NOK amount that was actually billed. Blank = no discount.
...
Oppgave kategori (multi_select) — Fiken product / service category. The FIRST selected label drives the invoice line's `description` (the bold first line on the printed invoice, e.g. "Næring - Interiør", "Print") AND its `incomeAccount` via `FIKEN_KATEGORI_TO_ACCOUNT` in [config.py](../src/gb_automations/config.py) (3020 for tjeneste, 3000 for vare, etc.). When blank the engine falls back to "Navn — Beskrivelse" for the line text and the default account (3020). Unknown labels also fall back to 3020 with a WARN log — add the kategori to `FIKEN_KATEGORI_TO_ACCOUNT` to route it explicitly.
