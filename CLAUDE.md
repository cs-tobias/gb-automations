...
**Single "Send faktura" button** on the Projects DB drives Notion → Fiken draft creation. **Bulletproof eligibility**: every Oppgave under the project lands on the draft unless it's explicitly excluded. The only skip rules are (1) `Fakturert status = Utgår` (operator excluded), (2) `Fakturert status = Fakturert` (already fully billed — nothing to add), (3) oppstart click on a row already at `Fakturert 50%` (nothing to add on this mode), (4) `Type = Korreksjonsrunde` (auto-generated admin sub-row from the Frame.io integration), or (5) slutt on a row where Pris was renegotiated below Fakturert beløp (Notion's `Budsjett` vs `Fakturert sum` rollups surface the over-bill; credit-note flow is a future feature). Everything else lands — non-discipline Type, missing Pris (kr 0 line), missing Kategori (free-text line with default `incomeAccount = 3020`). The Project's `Faktura status` (Notion `status` property type, not Select) picks the mode (`Til oppstartsfaktura` → bill 50% of every eligible Oppgave; `Til avslutningsfaktura` → bill the remainder). Per-Oppgave inclusion is the row's `Fakturert status` (also `status` type): `Ikke fakturert` / `Fakturert 50%` / `Fakturert` / `Utgår` — engine-written after a successful draft, except for `Utgår` which is operator-only. The reader (`notion_client.read_select_name`) falls back between Notion's `select` and `status` shapes so flipping a column type doesn't break reads; writes go through `set_oppgave_billed` / `set_project_faktura_status` which send the `status` wrapper.
