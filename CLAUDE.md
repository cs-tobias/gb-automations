...
## Fiken — Phase B v2 shipped (June 2026)

**Single "Send faktura" button** on the Projects DB drives Notion → Fiken draft creation. The Project's `Faktura status` (Notion `status` property type, not Select) picks the mode (`Til oppstartsfaktura` → bill 50% of every eligible Oppgave; `Til avslutningsfaktura` → bill the remainder). Per-Oppgave inclusion is the row's `Fakturert status` (also `status` type): `Ikke fakturert` / `Fakturert 50%` / `Fakturert` / `Utgår` — engine-written after a successful draft, except for `Utgår` which is operator-only. The reader (`notion_client.read_select_name`) falls back between Notion's `select` and `status` shapes so flipping a column type doesn't break reads; writes go through `set_oppgave_billed` / `set_project_faktura_status` which send the `status` wrapper.

**Notion is the invoice ledger.** Each Oppgave carries `Pris` (mutable — operators can renegotiate between oppstart and slutt), `Rabatt` (Notion `Percent` format — type `15` for 15%; API returns fraction `0.15`, engine consumes directly so do NOT divide by 100 again), and `Fakturert beløp` (engine-written running NOK total). Slutt computes `remaining = Pris × (1 − Rabatt) − Fakturert beløp`, so a renegotiated Pris flows through correctly. Rabatt is forwarded to Fiken on the line itself (`discount` field, percent 0–100) — the engine sends the un-discounted `unitPrice` so the customer sees "Pris kr X / Rabatt Y% / Sum kr Z" on the printed invoice rather than a pre-rabattert unit price. A project-level rollup of `Fakturert beløp` is the displayed `Fakturert sum`. Vår referanse = project title; Deres referanse = project `Faktura merkes` (rich_text). Line text is driven by the Oppgave's `Oppgave kategori` multi_select (first label wins): `description` (bold) = the Kategori label (e.g. `Næring - Interiør`, `Print`), `comment` (sub-line) = `Navn - Beskrivelse`. `incomeAccount` per line is also Kategori-driven via `FIKEN_KATEGORI_TO_ACCOUNT` in [config.py](src/gb_automations/config.py) (3020 tjeneste, 3000 vare, …; default 3020 + WARN on unknown). Rows with no Kategori fall back to `Navn — Beskrivelse` as the description and the default 3020 account. Customer resolved via one direct relation: Project → `Fakturamottaker` → Orgnr, with `/contacts` auto-creation on no-match when Orgnr is present (so a project can bill a different entity than the parent Kunder uses by default). Missing Orgnr (no Fakturamottaker linked, or Orgnr blank) is NOT a failure — engine links the draft to a shared "Mangler kunde" placeholder contact (auto-created on first use, cached in `fiken_placeholder_contacts` per `company_slug`; name configurable via `settings.fiken_placeholder_contact_name`). Fiken's API rejects drafts with no `customerId`, so the placeholder is required; operator picks/creates the real customer in Fiken's draft UI before clicking Send. Most recent draft URL lands in a single `Faktura utkast` column on the Project — transient by design (the URL points at a Fiken DRAFT specifically; once the draft is sent it becomes a real invoice on a different URL and this one 404s, which is the correct "graduated" signal). Setup: [docs/misc/fiken-integration.md](docs/misc/fiken-integration.md) (Phase B v2 section).

Phase C (Fiken → Notion sent-invoice/offer poller, replacing the existing Make automation) still not started; schema exists.
