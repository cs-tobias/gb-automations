...
`/assets/placeholder/{deliverable_page_id}.jpg`, which renders on demand from
...
JPEG (not PNG): Goldbox ships deliverables as JPG, so the V00 placeholder
matches the extension the studio's first real V01 upload replaces in Frame's
version stack.
...
plain black JPEG with a 200, never a 4xx/5xx: a failed fetch would otherwise
...
# Pre-baked solid-black JPEG used as a last-resort fallback when
...
_FALLBACK_JPG = render_placeholder(None, None)
...
@router.get("/placeholder/{deliverable_page_id}.jpg")
async def placeholder_jpg(deliverable_page_id: str) -> Response:
    text: str | None = None
    ...
    # preview. Fall back to the pre-baked solid-black JPEG so the endpoint
    # always returns valid JPEG bytes.
    try:
        jpg = await asyncio.to_thread(render_placeholder, text, bg_bytes)
    except Exception:
        logger.error(
            "🖼  placeholder: render_placeholder raised for %s — "
            "returning fallback JPEG (Frame would otherwise store an error body)",
            deliverable_page_id,
            exc_info=True,
        )
        jpg = _FALLBACK_JPG
    ...
    # jpg size → bytes were fine, problem is downstream; log line present
    # with jpg size == fallback size → renderer raised (see ERROR above).
    text_preview = (
        text if not text else (text[:60] + "…" if len(text) > 60 else text)
    )
    logger.info(
        "🖼  placeholder render for %s: text=%r bg_bytes=%d → jpg=%d bytes",
        deliverable_page_id,
        text_preview,
